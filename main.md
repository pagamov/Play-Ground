...
|----------|----------|----------|----------|----------|----------|----------|----------|
| необходимо рисовать, если направление потока снизу вверх и справа налево | массив | Линейный алгоритм | постановка задачи, выбор метода решения, проектирование программ | определение последовательности действий, ведущих к получению результатов | компилятор | цикл со счетчиком | сортировка |
| Блок autorun | сериализацией | закладка | персональная, корпоративная, государственная | предотвращает утечку информации с компьютера | взлом методом перебора паролей | заключается в его высокой чувствительности и точности при обнаружении аномалий в данных | Сетевой |
| вывода на экран информации из таблицы или запроса, основываясь на указанном пользователем формате | конструктор | атрибутом | Размером поля. Кол-ва Байт в переменной | Триггеры | B иерархических базах данных данные организованы в виде дерева | числовой | текстовый |
| Интеграция с другими приложениями Тематические слоя Трехмерную визуализацию данных запрашиваемых пользователем | С несколькими видами электронных сообщений . С сообщениями, которые фиксируют факт и содержание каждого этапа поставки. Они соответствуют шагам в документообороте между поставщиком и покупателем: заказ, отгрузка, приемка товара, счет на оплату . Только между двумя участниками. Покупатель и продавец | between systems or applications | нахождение и просмотр всевозможных документов в интернете | www.fizika.ru | Тактовая частота процессора – это количество тактов (циклов) в секунду, на которых работает центральное процессорное устройство (ЦПУ) компьютера, измеряемое в герцах (Гц) | прокси-сервер операционного центра | антенну и микрочип |
| система последовательных операций, которые необходимо выполнить для решения задачи | отладка | пооператорное выполнение программы | линковщика и компилятора | Алгоритм | набор элементарных знаков, используемых при составлении любых текстов, называют алфавитом языка | Семантическая | модификация |
| От тактовой частоты | составляющая компиляции, которая обрабатывает директивы или команды | для инициализации объектов класса | не имеет | оптоволоконная кабельная связь | составной объект, в который могут входить элементы различных типов | наследования производного класса от нескольких базовых или нескольких интерфейсов | усложняет процедуру комплексного тестирования |
| Компиляция | улучшение работы существующей программы | тестирование | проектирования | в разделе описания переменных | автоматическое составление машинной программы по исходной программе, записанной на языке программирования, выполняемое транслятором-компилятором; языки компилирующего типа сначала переводят весь текст программы в машинные коды, а уже затем полученный файл может быть запущен на выполнение | сложение, вычитание, умножение, целочисленное деление, нахождение остатка | разбиение алгоритма на конечное число простых шагов |
| модуляцией | Маршрутизация двухточечных соединенийМаршрутизация с помощью Протокола преобразования адресов ProxyДинамическая маршрутизацияС обнаружением сбоев в работе шлюзаСвязывание маршрутов Бесклассовая междоменная маршрутизация Маршрутизация с помощью виртуальных IP-адресов  Маршрутизация в системе несколькими разделами с помощью OptiConnect | мб/сек Кбайт/сек | Интерфейс сети | Уровень предоставления | нет не входят. к магистрали подключены только ЦП и RAM | метод линейного кодирования, при котором логические уровни «0» и «1» представляются разными уровнями напряжения или тока, и сигнал не возвращается к нулевому уровню между соседними битами | Высоким качеством связи |
| данные организовываются в виде отношений | компьютерный вирус логические мины внедрение в информационный обмен | неопределенных значений | обнаружения и защиты от несанкционированного изменения критически важных компонентов, отслеживания неизменности файлов, документов, реестра, конфигурации оборудования и других элементов информационной системы. Это механизм, необходимый для обеспечения безопасности и целостности информационной системы. | .mdb, .db | Инкапсуляция - Encapsulation | количество кортежей в отношении | выборку данных согласно заданным атрибутам |
| деструктор не содержит параметров | на использовании сервисов со стандартизированными интерфейсами | Friend function | САПР (система автоматизированного проектирования) | инкапсуляция | скобками, приоритетом операций, а при одинаковом приоритете ассоциативностью операций | невозможно создать объект абстрактного класса | DISCO |
| снятием с эксплуатации | каждая итерация соответствует работам по отдельному этапу проектирования программной системы | водопадная каскадная | синтаксическая | формулирование требований | анализ существующих аналогов | каскадную | передача свойств потомкам |
| сетка, узел, отношение + Автоматическая связность родителей и потомков. Потомок не может существовать без родителя Различные уровни Иерархии. Есть родитель и дети. Имеется строгое подчинение и уровни (в том числе уровни доступа) Древовидная структура. Частный случай связного графа  | наличие как вертикальных, так и горизонтальных иерархических связей | Изменении структуры ALTER или (при удалении поля) | первичный ключ может принимать нулевое значение | Предоставление обьектов в строго определенном формате | супер ключом | множество значений, которые могут принимать объекты программы, а также совокупность операций, допустимых над этими значениями | Список подстановки |
| ключ | символ * | непротиворечивости данных | сущностью | сетевых адаптеров | Виртуальным атрибутом | запрос | секретность закрытого сообщения определяется секретностью ключа |
| трафиком | Frame Relay | интегральная микросхема, которая выполняет поступающие на ее вход команды (например, вычисление) и управляет работой машины | Постоянная память | функциональной организации компьютера, которое включает в себя описание его компонентов, их взаимодействия и способов обмена данными. Это описание | модель, в которой потребителю предоставляется возможность использования прикладного программного обеспечения провайдера, работающего в облачной инфраструктуре и доступного из различных клиентских устройств или посредством тонкого клиента, например, из браузера или посредством интерфейса программы | Маршрутизация | интерфейсных разъемов |
